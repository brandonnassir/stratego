...
| `reports/phase18/phase18_rule_identity_errata_v1.json` | `ACCEPTED` (as a record) | E-P18-D003-RULES-1: the packet's 100 is a metadata error, the contract/engine/schedule/receipts carry 200, no rerun. O-P18-EVALRULES-1: `phase18_evaluation_contract_v1.json` names *training* rules (100) for future play lanes — **RESOLVED 2026-09-02 by the accepted amendment `P18-A001`** (battleless 200 for play evaluation); does not affect G2. |
| `reports/phase18/amendments/P18-A001_EVALUATION_RULE_IDENTITY.{md,json}` | **`ACCEPTED — OPERATIVE`** for future G3/G4 play evaluation (2026-09-02) | Proposed resolution of `O-P18-EVALRULES-1`. Recommends `EVALUATION_RULES` (battleless 200, absolute 4000, first player 0, context evaluation) as the single rule identity for future G3/G4 play evaluation, superseding one field — `cases_seeds_opponents.rule_version` — of the frozen evaluation contract, which is **not edited** (its digest is pinned in `P18-D001.json` and `phase18_agent1_handoff_v1.json`). Statistical claims corrected 2026-09-02: no power advantage for 200 over 100 is claimed or demonstrated; draws score 0.5 and do contribute to paired differences; a shorter limit only weakly increases or preserves battleless-draw terminations, with unknown effect on variance, effect size and power. Grounded in the engine constants, `match_spec` defaults, the frozen G1 contract, the driver and all 16,384 G1 receipts. Training rules (battleless 100) unchanged; prior G1/G2 results unchanged; **authorizes no G3 run**. Accepted by the operator 2026-09-02 at commit `959461f`; the delivered files are preserved as written (their own status line still reads proposed) and acceptance is recorded in `reviews/P18-A001_REVIEW.md`. |
| `reports/phase18/process_notes/P18-PN001_STAGE3_PREMATURE_PUBLICATION.md` | `RECORD` (append-only, 2026-09-02) | Process deviation: Stage 3 commit `69c97db` was pushed before review, contrary to the required hard stop; Stage 2 `cac7abf` was correctly published only after approval. `P18-A001` stayed `PROPOSED — NOT OPERATIVE` and `O-P18-EVALRULES-1` stayed open, so the premature push authorized no experiment and changed no active rule. History is **not** rewritten — `69c97db` is preserved as published and corrected forward. |
| `reports/phase18/reviews/P18-A001_REVIEW.md` | `ACCEPTED` (as a record) | Acceptance of `P18-A001` as **operative for future G3/G4 play evaluation**: the canonical `EVALUATION_RULES` token (battleless 200, absolute 4000, first player 0, context evaluation); supersedes only `cases_seeds_opponents.rule_version`; the frozen contract stays byte-identical at `6878bab6…` with both pins verifying; training/corpus rules stay at 100; G1/G2 results unchanged and both gates still closed; **authorizes no G3 run**; records the premature publication of `69c97db`, its forward correction at `959461f`, and the approved local == remote hash. |
| `reports/phase18/phase18_execution_storage_policy_v1.md` | `ADOPTED` (2026-09-02) | Phase 18 execution-storage policy: the main repository stays at `…/gpt_agent`; future detached worktrees go under `output/phase18/worktrees/` and future untracked runtime artifacts under `output/phase18/runtime/` (two narrowly scoped `.gitignore` entries — `output/` itself is deliberately not ignored, while the pre-existing global `*.pdf` rule still applies); tracked evidence stays where it is; every future launch manifest records its exact worktree and runtime paths; worktrees are created from a frozen source commit and verified clean before execution. Inventories the four historical G1/G2 worktrees (864 MiB total, all detached and Git-clean under ordinary status, with ignored content explicitly excluded from that claim), which **stay in place** because dozens of references in accepted tracked artifacts record their absolute paths, and which may not be moved or removed except via an approved archival migration that first inventories and preserves ignored or unique material. |
...
| `scripts/phase18_g2_raw_confirmation.py`, `tests/training/phase18/test_g2_raw_confirmation_driver.py` | **`ACCEPTED`** | The confirmation driver at `G2_RAW_SOURCE_COMMIT ccddceda2701`: reuses the G2 implementation unchanged (16/16 method files byte-identical to the G2 launch manifest, checked at freeze, verify and launch), derives every seed from the reviewed base commit and the namespace, records and re-verifies the optimum certificate, applies the frozen decision rule mechanically. Nine tests. |
| `reports/phase18/phase18_g2_raw_confirmation_contract_v1.json`, `phase18_g2_raw_confirmation_landscape_v1.json`, `phase18_g2_raw_confirmation_launch_manifest_v1.json` | **`ACCEPTED`** (frozen before any outcome) | Fresh table (seed 475578596360375388, digest `210352ef…`), exact optimum **53.031081** with LP-duality potentials recorded and re-verified by arithmetic, uniform mean −0.3423 SD 5.9529; fresh model/pool/outcome/evaluation/bootstrap seeds (audited against the G2 assay and both development smokes); raw actor primary, EMA telemetry; PROCEED/STOP/REVISE rule frozen; launch manifest from the clean worktree with zero outcomes in existence. |
| `reports/phase18/g2_raw_confirmation/` (verification, JUnit ×2, coverage, oracle, per-seed results, replay) | **`ACCEPTED`** | 86/86 evaluator + 131/131 setup tests; oracle PASS on this run's seed (loss terms 1.8e-15, gradients 6.7e-11); coverage 30/30; replay reproduces the landscape, first-period outcomes and all four evaluation endpoints of every seed bitwise. |
| `reports/phase18/phase18_g2_raw_confirmation_results_v1.json`, `phase18_g2_raw_confirmation_binding_v1.json`, `phase18_g2_raw_confirmation_decision_input_v1.json` | **`ACCEPTED`** | Zero integrity events; twelve complete finite 4,096-sample arrays. Raw actor: all seeds improved, pooled 95% [+5.2985, +5.5585], gap closure 10.648 / 10.007 / 9.599%, median **10.007%** (threshold 10%, met by 0.0073 percentage points). EMA telemetry: pooled 95% [+0.0116, +0.1386], median 0.119%. Eleven artifacts bind one source commit; the frozen rule returns PROCEED. |
| `reports/phase18/decisions/P18-D005.*`, `agent_05_report.md` | **`ACCEPTED`** — `PROCEED` (synthetic trainability portion of G2 only), accepted 2026-09-02 | The parity-correct raw learner reliably learns a fresh landscape within 64 updates; the practical-margin criterion is met at its edge and the packet says so. Authorizes designing the next gate only. Branch `phase18/g2-raw-confirmation` **published** at `d0fd36d9469a048dbbc9aa1a233f6006d78b0e8d` (local == remote). `agent_05_report.md` is unedited: its digest is pinned in the frozen `P18-D005.json` artifacts block, so its "awaiting review" line records the state at delivery; acceptance lives in the review record and the indexes. |
| `reports/phase18/reviews/P18-D005_REVIEW.md` | `ACCEPTED` (as a record) | Acceptance of `P18-D005`, the closure of Gate G2, the publication receipt (local == remote `d0fd36d9…`), the recorded reservation that the median 10.0073% gap closure clears the frozen 10% rule by only 0.0073 percentage points and is not robust evidence for a precise 10% effect, the frozen contract's contradictory inherited metadata with the fields that were authoritative, and erratum `E-P18-D005-CI-1` on the `be4fc84` commit message (upper endpoint +5.6072 vs the authoritative +5.5585; lower endpoint and decision unaffected). |
| `artifacts/phase18/g2_raw_confirmation_v1/` | `ENGINEERING` (git-ignored, canonical tree, 99 MB) | Per-seed outcome receipts, telemetry, held-out utility arrays (raw and EMA) and the three-object checkpoints; reproducible from the frozen contract. The G1 and G2 artifacts are untouched. |
