...
| `reports/phase18/amendments/P18-A001_EVALUATION_RULE_IDENTITY.{md,json}` | **`PROPOSED — NOT OPERATIVE`** (2026-09-02) | Proposed resolution of `O-P18-EVALRULES-1`. Recommends `EVALUATION_RULES` (battleless 200, absolute 4000, first player 0, context evaluation) as the single rule identity for future G3/G4 play evaluation, superseding one field — `cases_seeds_opponents.rule_version` — of the frozen evaluation contract, which is **not edited** (its digest is pinned in `P18-D001.json` and `phase18_agent1_handoff_v1.json`). Grounded in the engine constants, `match_spec` defaults, the frozen G1 contract, the driver and all 16,384 G1 receipts. Training rules (battleless 100) unchanged; prior G1/G2 results unchanged; **authorizes no G3 run**; operative only after review. |
