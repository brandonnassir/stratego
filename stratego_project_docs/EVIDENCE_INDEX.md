...
| `reports/phase18/phase18_execution_storage_policy_v1.md` | `ADOPTED` (2026-09-02) | Phase 18 execution-storage policy: the main repository stays at `…/gpt_agent`; future detached worktrees go under `output/phase18/worktrees/` and future untracked runtime artifacts under `output/phase18/runtime/` (two narrowly scoped `.gitignore` entries — `output/` itself is deliberately not ignored); tracked evidence stays where it is; every future launch manifest records its exact worktree and runtime paths; worktrees are created from a frozen source commit and verified clean before execution. Inventories the four historical G1/G2 worktrees (864 MiB total, all detached and clean at their frozen commits), which **stay in place** because 53 references in 36 tracked files record their absolute paths, and which may not be moved or removed except via an approved archival migration. |
