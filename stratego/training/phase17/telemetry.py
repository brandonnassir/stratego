...
class TelemetryWriter:
    ...
    def resume(cls, position: dict, *, run_id: str) -> "TelemetryWriter":
        ...
        than the recorded offset. That excess is a row the resumed run is about
        to produce again, so it is truncated back rather than appended past: an
        append would leave two rows for one iteration and a later reader could
        not tell which one the training actually used.
        """
...
def telemetry_schema() -> dict:
    ...
    return {
        ...
        "resume_rule": (
            ...
            "offset; a longer tail is truncated back, a shorter one is refused"
        ),
    }
