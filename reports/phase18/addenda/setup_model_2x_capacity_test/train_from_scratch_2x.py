...
OUT = ROOT / "output/phase18/runtime/addendum_library_setup_from_scratch_2x"
...
# ---- 2x model: 8 decoder blocks instead of 4 (everything else identical) ----------------
from stratego.training.phase18.setup_model import Phase18SetupModel as _M
from stratego.training.phase18 import setup_learning as _sl
BIG_BLOCKS = 8
def build_big(device="cpu", seed=None):
    if seed is not None:
        st_ = torch.random.get_rng_state(); torch.manual_seed(int(seed))
        try: m = _M(blocks=BIG_BLOCKS)
        finally: torch.random.set_rng_state(st_)
    else: m = _M(blocks=BIG_BLOCKS)
    return m.to(device)
_sl.build_setup_model = build_big          # EMA.as_model, save/load_checkpoint now build the 2x model

NS = "phase18_addendum_library_setup_from_scratch_v1"
SEED_INDEX = 1
RUN_ID = "ADDENDUM-LIB-SETUP-2X-2026-A"
PERIODS = 256
...
def main():
    ...
    if state_path.exists() and not a.smoke:
        ...
        model = build_big(device="cpu", seed=setup_model_init_seed(NS, SEED_INDEX))
        trainer = SetupTrainer(model, config, namespace=NS, seed_index=SEED_INDEX); start = 1
        if not a.smoke:
            trainer.save_checkpoint(OUT / "ckpt_0000")
        log(f"2x model: {BIG_BLOCKS} blocks, {sum(p.numel() for p in model.parameters())} parameters"); log(f"fresh init raw {state_dict_digest(model)[:12]} ema {state_dict_digest(trainer.ema.as_model(device='cpu'))[:12]} | {len(canon)} library formations | {len(cells)} cells")
