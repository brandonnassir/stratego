...
ROOT=/Users/brandonwashington/Dev/Github/stratego/gpt_agent; H=$ROOT/output/phase18/runtime/addendum_coadapt; LOG=$H/logs/memory_watch_375.log
FREE_MIN=15; WARN_AT=30; SWAP_MAX_GB=6; low=0; warned=0
...
say "started"
while true; do
  [ -f "$H/DONE_375" ] && { say "chain finished; exiting"; exit 0; }
  reap; f=$(freepct); s=$(swapgb); f=${f:-100}; s=${s:-0}
  stage="idle"; pgrep -f "coadapt_lineage.py" >/dev/null && stage="train"; pgrep -f "coadapt_eval_final.py" >/dev/null && stage="eval"
  ...
  if { [ "$low" -ge 2 ] || [ "$hit" -eq 1 ]; } && [ "$stage" != "idle" ]; then
    ...
    for pat in coadapt_lineage.py coadapt_eval_final.py; do for p in $(pgrep -f "$pat"); do for c in $(pgrep -P "$p"); do kill -KILL "$c" 2>/dev/null; done; kill -KILL "$p" 2>/dev/null; done; done
