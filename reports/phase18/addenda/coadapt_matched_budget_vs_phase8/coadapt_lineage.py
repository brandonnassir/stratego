...
def main():
    p = argparse.ArgumentParser(); p.add_argument("--resume", action="store_true"); p.add_argument("--periods", type=int, default=None); p.add_argument("--horizon", type=int, default=256); a = p.parse_args()
    ...
    config = dataclasses.replace(driver.production_config("control", c1_device="mps"), run_id=RUN_ID, periods=a.horizon)
    legacy = dataclasses.replace(config, periods=256)     # the identity the pre-extension bundles were written under
    if a.horizon != 256:
        log(f"HORIZON EXTENSION: periods={a.horizon} (Phase 8 budget {a.horizon*64} C1 updates); the only config difference from the 256-period v4 lineage is the horizon; C1 lr schedule is warmup-then-constant, so training dynamics are unchanged")
    ...
    if a.resume:
        try:
            runner = gp.LineageRunner.resume(config, bundle_directory=None, **kw)       # a bundle written under the current horizon
        except Exception as first:
            if a.horizon == 256: raise
            log(f"no bundle verifies under the {a.horizon}-period identity ({type(first).__name__}); resuming from the newest bundle under the 256-period identity it was written with")
            object.__setattr__(config, "matched_digest", legacy.matched_digest); object.__setattr__(config, "config_digest", legacy.config_digest)
            try:
                runner = gp.LineageRunner.resume(config, bundle_directory=None, **kw)
            finally:
                object.__delattr__(config, "matched_digest"); object.__delattr__(config, "config_digest")     # new bundles carry the true horizon identity
            log("DEVIATION RECORDED: source bundle verified under its own (256-period) identity; all bundles written from here on carry the 375-period identity")
