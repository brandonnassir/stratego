...
SEED_INDEX = 1
RUN_ID = "ADDENDUM-HALF-ROSTER-" + (sys.argv[sys.argv.index("--half") + 1] if "--half" in sys.argv else "A") + "-2026"
PERIODS = 256
...
def main():
    p = argparse.ArgumentParser(); p.add_argument("--smoke", action="store_true"); p.add_argument("--half", default="A"); p.add_argument("--periods", type=int, default=PERIODS); a = p.parse_args()
    ...
    bots = list(HALVES[HALF])
    cells = [(r, b) for r in bots for b in bots if r != b]
    assert len(cells) == 12
    log(f"HALF {HALF}: training roster {bots}; {len(cells)} ordered pairs; games/period unchanged")
