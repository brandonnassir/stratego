...
NAMESPACE = driver.NAMESPACE; NS = "phase18_addendum_library_setup_from_scratch_v1"; P = 224

def log(m):
    line = f"[eval-p224 {time.strftime('%H:%M:%S')}] {m}"; print(line, flush=True)
    with open(W / "logs/eval_p224.log", "a") as f: f.write(line + "\n")
...
def main():
    ...
    movers = {"coadapt_p224": W / "runtime/control/bundles/bundle_0224", "g3control_p224": RT / "control/bundles/bundle_0224"}
    plan = [("coadapt_p224", "ckpt_1024"), ("g3control_p224", "ckpt_1024"), ("coadapt_p224", "library"), ("coadapt_p224", "g3_init")]
    rows, digests = {}, {}
    ...
    for mover, arm in plan:
        ...
        assert int(man["period"]) == P and man["components"]["c1"]["global_step"] == 64 * P
        ...
        try:
            ...
            t = time.perf_counter(); results, reports = ev.play_chunks(matches, W / "eval_p224" / mover / arm / "games", runner, chunk_units=64, label=f"{mover}@{arm}", log=log)
...
def analyse(rows, cases, digests):
    v3 = {arm: [json.loads(l) for l in (V3 / f"rows_{arm}.jsonl").read_text().splitlines() if l.strip()] for arm in ("ckpt_1024", "library")}
    ...
    A = {"movers": digests, "ewr": {f"{m}@{a}": sum(o["candidate_score"] for o in v)/len(v) for (m, a), v in rows.items()}, "ewr_control_p256_v3": {a: sum(o["candidate_score"] for o in v)/len(v) for a, v in v3.items()}, "contrasts": {}}
    def C(label, ra, rb):
    ...
    if ("coadapt_p224", "ckpt_1024") in rows and ("g3control_p224", "ckpt_1024") in rows: C("PRIMARY budget-matched: coadapt_p224 - g3control_p224 @ ckpt_1024", rows[("coadapt_p224", "ckpt_1024")], rows[("g3control_p224", "ckpt_1024")])
    if ("coadapt_p224", "ckpt_1024") in rows: C("coadapt_p224 - g3control_p256(v3) @ ckpt_1024", rows[("coadapt_p224", "ckpt_1024")], v3["ckpt_1024"])
    if ("g3control_p224", "ckpt_1024") in rows: C("g3control_p224 - g3control_p256(v3) @ ckpt_1024 (budget effect)", rows[("g3control_p224", "ckpt_1024")], v3["ckpt_1024"])
    if ("coadapt_p224", "library") in rows: C("coadapt_p224 - g3control_p256(v3) @ library", rows[("coadapt_p224", "library")], v3["library"])
    if ("coadapt_p224", "g3_init") in rows: C("coadapt_p224 - control_final(p256) @ g3_init", rows[("coadapt_p224", "g3_init")], v3["g3_init"])
    if ("coadapt_p224", "ckpt_1024") in rows and ("coadapt_p224", "library") in rows: C("coadapt_p224: ckpt_1024 - library", rows[("coadapt_p224", "ckpt_1024")], rows[("coadapt_p224", "library")])
    (W / "analysis_p224.json").write_text(json.dumps(A, indent=1, sort_keys=True) + "\n")
